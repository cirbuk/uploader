        ...
        if (chunkCount > 1) {
          ...
          Object.keys(urls).map((key, index) => {
            ...
            const file = chunksToBeUploaded[index];
              ...
              eventHandler({ type: "CHUNK_UPLOAD_PROGRESS",
              ...
              if (url) {
                ...
                .catch(exception => {
                  eventHandler({
                    ...
                    payload: { message: errorMessages.FILE_UPLOAD_FAILED, exception, taskId: tempIds[0], chunkTaskId: tempTaskId }
                  });
                });
              } else if (!isFileTypeSupported(extension)) {
                eventHandler({ type: "GET_FILE_UPLOAD_URL_FAILED", payload: { message: errorMessages.EXTENSION_NOT_SUPPORTED, taskId: tempIds[0] } });
              } else {
                eventHandler({ type: "GET_FILE_UPLOAD_URL_FAILED", payload: { message: errorMessages.GET_FILE_UPLOAD_URL_FAILED, taskId: tempIds[0] } });
              }
            });
        } else {
